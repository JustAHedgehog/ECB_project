function Fz = ECB_AxialForce(ECB, rpm, g_curr)
    % 幾何尺寸 (m)
    R0 = ECB.r_yi;        % 銅盤內徑
    Rm = ECB.r_av;        % 平均半徑
    L = ECB.l_m;          % 磁鐵徑向長度
    ...
    alpha_p = ECB.PM_ratio;     % 極弧係數 (Pole-arc to pole-pitch ratio)
    p = ECB.p;                  % 極對數
    Br = ECB.B_r;                % 剩磁 (T)
    sigma = ECB.sigma;          % 銅導電率 (S/m)
    
    N_harmonics = ECB.N_harm;
    K_harmonics = ECB.K_bessel;
    ...
    for n_idx = 1:N_harmonics
        ...
        for k_idx = 1:K_harmonics
            ...
            % 注意 γ_nk 內部含有虛數 1i
            gamma_nk = sqrt((n * pi / H)^2 + (k * pi / tau)^2 + 1i * sigma * ECB.mu_0 * Vx * (k * pi / tau));
            
            % --- 步驟 2：計算磁化展開係數 M_nk (公式 19) ---
            % 注意：推導常數項為 16 * Br / (pi^2 * mu0 * n * k)
            M_nk = (16 * Br) / (pi^2 * ECB.mu_0 * n * k) * sin(k * alpha_p * pi / 2) * sin(n * (pi/2) * (L / H));
    ...
    Fz = 0.25 * ECB.mu_0 * p * tau * H * real(Summation_Term); 
end